...
import { assertPaymentWatermarkInput, assertSamePaymentEventIdentity, parsePaymentWatermarkReceipt,
  type PaymentWatermarkInput, type PaymentWatermarkResult } from "@/lib/payments/payment-watermark-contract";
...
export async function applyPaymentEventWatermark(
  input: PaymentWatermarkInput,
  dependencies: StripeWebhookEventStorageDependencies = eventStorage,
): Promise<PaymentWatermarkResult> {
  assertPaymentWatermarkInput(input);
  const incoming = buildPaymentEventWatermark(input);
  if (!canUseDurablePaymentStorage()) {
    if (requiresDurablePaymentStorage()) throw new Error("payment_event_watermark_storage_unavailable");
    const seen = memoryPaymentEventIdentities.get(input.eventId);
    if (seen) assertSamePaymentEventIdentity(seen, incoming);
    const decision = decidePaymentEventOrdering(
      memoryPaymentEventWatermarks.get(input.subjectKey) ?? null, incoming,
    );
    memoryPaymentEventIdentities.set(input.eventId, incoming);
    if (decision.accepted) memoryPaymentEventWatermarks.set(input.subjectKey, decision.next);
    return { ...decision, currentEventId: decision.next.eventId, currentKind: decision.next.kind };
  }
  ...
  return parsePaymentWatermarkReceipt(data, input);
}
