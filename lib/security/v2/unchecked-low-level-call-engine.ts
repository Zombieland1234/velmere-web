...
const LOW_LEVEL_CALLS = new Set(["CALL", "CALLCODE", "DELEGATECALL"]);

/**
 * High-specificity SWC-104 detector.
 *
  ...
  for (const block of cfgResult.cfg.blocks.values()) {
    for (let i = 0; i < block.instructions.length - 1; i++) {
      ...
      return [{
        ...
        limitations: ["IMMEDIATE_CALL_STATUS_DISCARD_ONLY", "NO_EXECUTED_TARGET_CALL"],
        ...
        proofOfConcept: {
          summary: "UNEXECUTED bytecode proof: CALL-family success word is immediately consumed by POP.",
          sequence: [
