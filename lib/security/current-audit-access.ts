...
export async function resolveCurrentAuditAccess(
  ...
  const advancedAvailable = getVlmCurrentSkuTruth("advanced").decision !== "NOT_FOR_SALE";

  if (!entitlementId) {
    // Historical verification is not current access: regeneration requires an active grant.
    // Current SKU truth outranks any legacy/stale entitlement row.
  ...
  const entitlementCheck = await verifyVlmPaidSurfaceEntitlementById({
    ...
    entitlementId,
    allowedProductIds: advancedAvailable
      ? ["vlm_pro_audit_review", "vlm_advanced_audit_human_review"]
