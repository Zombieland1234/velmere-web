import { proveFunctionMutex } from "./solidity-guard-proof.mjs";
// PASS36 A102R44P15
...
export const ANALYZER_CLASS = "STRUCTURED_TOKEN_CONTROL_FLOW_V4_BOUNDED_MUTEX_NOT_COMPILER_AST";
...
function extractContracts(code) {
  ...
  for (const match of code.matchAll(pattern)) {
    ...
    if (close < 0) continue;
    rows.push({ name: match[1], start, open, close, hasInheritance: /\bis\b/.test(code.slice(start, open)), code: code.slice(open + 1, close) });
  }
  return rows;
}
...
function hasCheckedLowLevelCall(body) {
  if (/\b(?:require|assert)\s*\([^;]{0,320}\.(?:call|callcode|delegatecall|staticcall)\s*(?:\{|\()/i.test(body)) return true;
  if (/\bif\s*\(\s*!?[^;]{0,320}\.(?:call|callcode|delegatecall|staticcall)\s*(?:\{|\()/i.test(body)) return true;
  const assignment = /(?:\(\s*bool\s+([A-Za-z_]\w*)\s*,?[^)]*\)|\bbool\s+([A-Za-z_]\w*))\s*=\s*[^;]*\.(?:call|callcode|delegatecall|staticcall)\b/gi;
  for (const match of body.matchAll(assignment)) {
    const variable = match[1] ?? match[2];
    if (!variable) continue;
    const escaped = variable.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const checked = new RegExp(`\\b(?:require|assert)\\s*\\(\\s*!?\\s*${escaped}\\b|\\bif\\s*\\(\\s*!?\\s*${escaped}\\b`, "i");
    if (checked.test(body.slice((match.index ?? 0) + match[0].length))) return true;
  }
  return false;
}

function hasCheckedSend(body) {
  if (/\b(?:require|assert)\s*\([^;]{0,240}\.send\s*\(/i.test(body)) return true;
  if (/\bif\s*\(\s*!?\s*[^)]*\.send\s*\([^)]*\)\s*\)\s*\{?\s*(?:throw\b|revert\s*\()/i.test(body)) return true;
  return /\bif\s*\(\s*[^)]*\.send\s*\([^)]*\)\s*\)[\s\S]{0,320}\belse\s*\{?\s*(?:throw\b|revert\s*\()/i.test(body);
}
...
function interactionRows(body, { includeTypedCallbacks = false } = {}) {
  ...
  const patterns = [
    ...
    ["ether-send", /\.\s*send\s*\(/g],
    ["ether-transfer", /\.\s*transfer\s*\(/g],
...
function hasReentrancyGuard(fn, modifiers, stateVariables, functions, contracts) {
  return proveFunctionMutex(fn, modifiers, stateVariables, functions, contracts);
}
...
export function analyzeReentrancyGuardCoverage(source) {
  ...
  for (const fn of functions) {
    ...
    const interactions = interactionRows(fn.body, { includeTypedCallbacks: true });
    const effects = stateEffectRows(fn.body, stateVariables);
    const firstInteraction = interactions[0];
    const postInteractionEffect = firstInteraction
      ? effects.find((effect) => effect.index > firstInteraction.index)
      : null;
    if (!firstInteraction || !postInteractionEffect) continue;
    rows.push({
      contractName: fn.contractName,
      functionName: fn.name || "<fallback>",
      line: fn.line,
      guarded: hasReentrancyGuard(fn, modifiers, stateVariables, functions, contracts),
      interactionKind: firstInteraction.kind,
      effectKind: postInteractionEffect.kind,
    });
  }

  const guardedPaths = rows.filter((row) => row.guarded).length;
  const unguardedPaths = rows.length - guardedPaths;
  return {
    analyzerClass: ANALYZER_CLASS,
    supportedPaths: rows.length,
    guardedPaths,
    unguardedPaths,
    allSupportedPathsGuarded: rows.length > 0 && unguardedPaths === 0,
    rows,
    limitations: [
      "Guard coverage applies only to externally callable source paths recognized by the bounded structured analyzer.",
      "No source map, compiler AST path feasibility, delegate/callback target identity or runtime exploit proof is implied.",
    ],
  };
}
...
export function analyzeSolidityStructuredSignals(source) {
  ...
  for (const fn of functions) {
    ...
    if (/\.(?:call|callcode|delegatecall|send)\s*(?:\{|\()/.test(body) && !hasCheckedLowLevelCall(body)) {
      const sendChecked = hasCheckedSend(body);
    ...
    const postInteractionEffect = firstInteraction
      ? effects.find((effect) => effect.index > firstInteraction.index)
      : null;
    const stipendInteraction = firstInteraction && (firstInteraction.kind === "ether-send" || firstInteraction.kind === "ether-transfer");
    if (firstInteraction && !stipendInteraction && postInteractionEffect && !hasReentrancyGuard(fn, modifiers, stateVariables, functions, contracts)) {
    ...
    if (!hasReentrancyGuard(fn, modifiers, stateVariables, functions, contracts) && effects.length > 0) {
