...
const baselineSha='9a176fe2e844ccf7166bf1bd7552f1bec79584cb';
const baseline=await lane('C12',baseRoot,baselineSha,inputs);
const candidate=await lane('C13',candidateRoot,process.env.GITHUB_SHA??'LOCAL_UNCOMMITTED',inputs);
...
const repeat=await lane('C13-stability',candidateRoot,process.env.GITHUB_SHA??'LOCAL_UNCOMMITTED',inputs.slice(0,25));
...
for(const name of ['C12','C13','C13-stability'])fs.unlinkSync(path.join(corpusDir,`${name}-engine.cjs`));
