...
def main() -> int:
    ...
    if not isinstance(files, list) or len(files) != len(set(files)) or len(files) != 56:
        raise ValueError('Expected the frozen 56-file test manifest without duplicates.')
    ...
    checks = [
        ...
        ('regressions', ['node', 'node_modules/tsx/dist/cli.mjs', '--test',
                         '--test-reporter=tap', '--test-concurrency=2', *files], 900),
