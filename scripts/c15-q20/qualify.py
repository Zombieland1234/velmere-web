...
def main() -> int:
    ...
    parser.add_argument('--candidate', default='C15-Q20-local')
    ...
    files = json.loads((root/'scripts/c15-q20/TEST_FILES.json').read_text())
    if len(files) != 67 or len(files) != len(set(files)):
        raise ValueError('Expected the reviewed, unique 67-file scope.')
    for name in files:
        path = (root/name).resolve()
        if not path.is_relative_to(root) or not path.is_file():
            raise ValueError('Missing or unsafe test file: '+name)
    expected = 1183 + 44 + 42 + 10
    ...
    for name in ('GITHUB_SHA', 'C6_SOURCE_SHA', 'GIT_DIR', 'GIT_WORK_TREE',
                 'PGDATABASE', 'PGHOST', 'PGUSER', 'Q12_DISPOSABLE_ACK', 'VELMERE_DEPLOYMENT_ENV', 'VELMERE_STAGING_SUPABASE_PROJECT_REF'):
        env.pop(name, None)
    checks = [
        ('worker', ['node','scripts/c11/build-audit-worker.mjs'], 180),
        ('regressions', ['node','node_modules/tsx/dist/cli.mjs','--test','--test-concurrency=2','--test-reporter=tap',*files], 900),
    ...
    checks += [
        ...
        (('build-monolithic', ['npm','run','build'], 900) if args.build_strategy=='monolithic'
         else ('build-staged', ['python','scripts/c15-q16/staged-build.py','--out',str(out/'staged-build'),'--candidate',args.candidate], 2700)),
    ]
