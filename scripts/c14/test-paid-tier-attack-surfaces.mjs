...
has(workspace, "p_operation in('READ','RESTORE')", "workspace READ/RESTORE must inspect stored tier");
has(workspace, "v_latest.tier not in('pro','advanced')", "stored workspace tier must be validated");
has(workspace, "velmere_r7_shield_pro_has_paid_entitlement_v1(v_latest.tier)", "restore/read must require a current entitlement for the stored tier");
has(workspace, "and workspace_id=p_workspace_id and account_id=v_account", "guessed workspace IDs must remain owner-bound");
