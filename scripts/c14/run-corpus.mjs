...
const baselineSha='4cb45bbcf910f0517d4a5d265682cd2f7e4e41df';
const baseline=await lane('C13',baseRoot,baselineSha,inputs);
...
const repeat=await lane('C14-stability',candidateRoot,process.env.GITHUB_SHA??'LOCAL_UNCOMMITTED',inputs.slice(0,25));
...
for(const name of ['C13','C14','C14-stability'])fs.unlinkSync(path.join(corpusDir,`${name}-engine.cjs`));
