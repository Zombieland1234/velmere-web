"""Qualify the frozen local Q7 scope without claiming a remote commit or release GO."""
...
def main() -> int:
    ...
    files = json.loads((root / 'scripts/c15-q7/TEST_FILES.json').read_text())
    if not isinstance(files, list) or len(files) != len(set(files)) or len(files) != 54:
        raise ValueError('Expected the frozen 54-file test manifest without duplicates.')
