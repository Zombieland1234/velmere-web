"""Qualify the frozen local Q9 scope without claiming a remote commit or release GO."""
...
def main() -> int:
    ...
    files = json.loads((root / 'scripts/c15-q9/TEST_FILES.json').read_text())
    if not isinstance(files, list) or len(files) != len(set(files)) or len(files) != 60:
        raise ValueError('Expected the frozen 60-file test manifest without duplicates.')
