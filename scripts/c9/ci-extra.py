...
out=Path('/tmp/c9-evidence');out.mkdir(exist_ok=True);sha=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip();assert sha==os.environ['GITHUB_SHA']
rows=[]
...
run('product-manifest',['node_modules/.bin/tsx','-e',"import{VLM_CANONICAL_CUSTOMER_PRODUCTS as products}from'./lib/product/vlm-canonical-product-topology';import fs from'node:fs';import{createHash}from'node:crypto';const p='lib/product/vlm-canonical-product-topology.ts';fs.writeFileSync('/tmp/c9-evidence/PRODUCT_MANIFEST.json',JSON.stringify({sourceSha:process.env.GITHUB_SHA,at:new Date().toISOString(),path:p,sha256:createHash('sha256').update(fs.readFileSync(p)).digest('hex'),catalogCount:products.length,activeOrSellableCount:'NOT_ASSUMED',products},null,2));"])
setup='''set -euo pipefail
mkdir -p /tmp/c9-scan /tmp/c9-tools
git archive HEAD | tar -x -C /tmp/c9-scan
curl --fail --silent --show-error --location --max-time 60 https://github.com/gitleaks/gitleaks/releases/download/v8.30.1/gitleaks_8.30.1_checksums.txt -o /tmp/c9-tools/checksums.txt
curl --fail --silent --show-error --location --max-time 90 https://github.com/gitleaks/gitleaks/releases/download/v8.30.1/gitleaks_8.30.1_linux_x64.tar.gz -o /tmp/c9-tools/gitleaks_8.30.1_linux_x64.tar.gz
cd /tmp/c9-tools
grep ' gitleaks_8.30.1_linux_x64.tar.gz$' checksums.txt | sha256sum -c -
tar -xzf gitleaks_8.30.1_linux_x64.tar.gz gitleaks
'''
if run('scanner-setup',['bash','-c',setup])==0:run('gitleaks-source',['/tmp/c9-tools/gitleaks','dir','/tmp/c9-scan','--redact=100','--no-banner','--no-color','--ignore-gitleaks-allow','--max-archive-depth','2','--max-decode-depth','2','--report-format','json','--report-path',str(out/'GITLEAKS_REDACTED.json')])
...
node_modules/.bin/playwright install --with-deps chromium > /tmp/c9-evidence/browser-install.log 2>&1
node_modules/.bin/next start --hostname localhost > /tmp/c9-evidence/next-runtime.log 2>&1 &
...
timeout 300 node scripts/c6/browser-observations.mjs http://localhost:3000 /tmp/c9-evidence/browser > /tmp/c9-evidence/browser.log 2>&1
timeout 90 node scripts/c6c/provider-outage-browser.mjs /tmp/c9-evidence/failures > /tmp/c9-evidence/controlled-failures.log 2>&1
timeout 90 node scripts/c6d/reference-pdf-browser.mjs /tmp/c9-evidence/pdf > /tmp/c9-evidence/pdf-browser.log 2>&1
'''
...
src=Path('/tmp/c9-source');src.mkdir(exist_ok=True);files=[];excluded=[]
omit={'lib/security/pro-audit-pdf/embedded-font-data.ts','r7-runtime/external-assets/manrope-pdf-latin-plus-ext.ttf'}
with zipfile.ZipFile(src/'VELMERE_SOURCE_C9.zip','w',zipfile.ZIP_DEFLATED) as z:
...
subprocess.run(['git','diff','0609ef1c5aeecfab6de3ada8144efaa089064c71','HEAD','--','components','public','app/**/*.css'],stdout=(out/'ui-source-diff.txt').open('w'),check=True)
