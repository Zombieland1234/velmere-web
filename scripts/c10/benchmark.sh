#!/usr/bin/env bash
set -euo pipefail
mkdir -p /tmp/c10-benchmark /tmp/c10-base /tmp/cgt
exec > >(tee /tmp/c10-benchmark/benchmark.log) 2>&1
printf 'candidate=%s\n' "$GITHUB_SHA"
npm ci --ignore-scripts --no-fund
# Offline comparison uses exact C8 tree, and never executes the corpus contracts.
git archive 80385a0bc9dbcff17c2a42e78c6dfc0f24ab407d | tar -x -C /tmp/c10-base
ln -s "$PWD/node_modules" /tmp/c10-base/node_modules
...
python3 scripts/c10/prepare-cgt.py /tmp/cgt /tmp/c10-benchmark
node scripts/c10/run-corpus.mjs /tmp/c10-base "$PWD" /tmp/c10-benchmark
