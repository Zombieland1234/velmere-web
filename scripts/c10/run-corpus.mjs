...
const baselineSha='80385a0bc9dbcff17c2a42e78c6dfc0f24ab407d';
const baseline=await lane('C9',baseRoot,baselineSha,inputs);
const candidate=await lane('C10',candidateRoot,process.env.GITHUB_SHA??'LOCAL_UNCOMMITTED',inputs);
...
const repeat=await lane('C10-stability',candidateRoot,process.env.GITHUB_SHA??'LOCAL_UNCOMMITTED',inputs.slice(0,25));
...
for(const name of ['C9','C10','C10-stability'])fs.unlinkSync(path.join(corpusDir,`${name}-engine.cjs`));
