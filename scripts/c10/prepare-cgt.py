...
manifest={'schema':'velmere.c10.external-runtime-regression.v1','recordedAt':datetime.datetime.now(datetime.timezone.utc).isoformat(),'datasetRepository':'gsalzer/cgt','datasetSha':PIN,'csvSha256':hashlib.sha256(raw).hexdigest(),'allCasesPreviouslyObservedInC9':True,'selectionRule':'ALL eligible unique decoded-runtime SHA256 in ascending order; all inputs were already observed in C9; regression replay, NOT a new holdout' ,'sourceSelectionRule':'First existing mapped source fingerprint in ascending order, <=500000 bytes; no compiler identity proof','csvRows':len(rows),'eligibleUniqueRuntimeHashes':len(unique),'selectedUniqueRuntimeHashes':len(public),'selectionDigestSha256':hashlib.sha256(('\n'.join(e['runtimeSha256'] for e in public)+'\n').encode()).hexdigest(),'previouslySeenDevelopmentCases':min(1200,len(public)),'previouslySeenValidationCases':max(0,len(public)-1200),'countingRule':'one contract runtime per SHA256, not assessment rows, reruns, names or mutations','excluded':excluded,'cases':public,'limitations':['Not the unavailable original R16 ledger','Not 1200 vulnerability classes or independently audited results','Weakness labels apply only to the specified SWC/property; absent labels do not mean safe','Conflicting consolidated labels excluded from confusion matrices','Historical external corpus; no live target or transaction is executed','Third-party source files are not redistributed; licensing remains with original sources']}
