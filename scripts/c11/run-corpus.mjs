...
const baselineSha='8841c625ede6452de55bd57c1024759d6dc53845';
const baseline=await lane('C10',baseRoot,baselineSha,inputs);
const candidate=await lane('C11',candidateRoot,process.env.GITHUB_SHA??'LOCAL_UNCOMMITTED',inputs);
...
const repeat=await lane('C11-stability',candidateRoot,process.env.GITHUB_SHA??'LOCAL_UNCOMMITTED',inputs.slice(0,25));
...
fs.writeFileSync(path.join(corpusDir,'COMPARISON.json'),JSON.stringify({manifestSha256,uniqueCases:inputs.length,allCasesPreviouslyObservedInC9:true,newUniqueCases:0,repeatedExecutionsNotNewCases:25,baseline:baseline.summary,candidate:candidate.summary,changedOutputs:changes,stabilitySubset:{count:25,unstable}},null,2));
...
for(const name of ['C10','C11','C11-stability'])fs.unlinkSync(path.join(corpusDir,`${name}-engine.cjs`));
