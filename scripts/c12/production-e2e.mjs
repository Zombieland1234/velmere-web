...
try{
 ...
 await record('missing-trusted-proxy-is-503',async()=>{const {status,body}=await getJson('http://127.0.0.1:3100',{'x-forwarded-for':'203.0.113.11','x-vercel-forwarded-for':'203.0.113.11'});assert(status===503,'unsigned bypass');return{http:status,error:body.mode??body.error};});
 await record('forged-hmac-is-503',async()=>{const {status}=await getJson('http://127.0.0.1:3100',{'x-velmere-proxy-address':'203.0.113.11','x-velmere-proxy-time':String(Date.now()),'x-velmere-proxy-signature':'0'.repeat(64)});assert(status===503,'forged assertion accepted');return{http:status};});
 ...
 for(const [name,width,height]of[['desktop',1440,1000],['mobile',390,844]])await record(`production-runtime-ssr-${name}`,async()=>{
  ...
  try{const r=await page.goto(`http://127.0.0.1:3200/en/security/audits/report/${target}?chainId=1&analysisMode=runtime&tier=basic`,{waitUntil:'domcontentloaded',timeout:25000});
   assert(r?.status()===200,'ssr_http');assert(await page.locator('.audit-canonical-view').count()===1,'ssr_report_missing');const text=await page.locator('body').innerText();assert(!/ANALYSIS_UNAVAILABLE/.test(text),'ssr_unavailable');await page.screenshot({path:path.join(out,`RUNTIME_${name}.png`)});assert(errors.length===0,'page_exception');return{http:r.status(),screenshot:`RUNTIME_${name}.png`,pageErrors:errors,bodySha256:createHash('sha256').update(text).digest('hex')};}finally{await page.close();}
 });
 await record('production-anonymous-pro-json-still-denied',async()=>{const r=await fetch(`http://127.0.0.1:3200/api/audit/report?${query.replace('tier=basic','tier=pro')}`,{signal:AbortSignal.timeout(10000)});await r.body?.cancel();assert(r.status===401,'pro_bypass');return{http:r.status};});
 await record('production-anonymous-advanced-pdf-still-denied',async()=>{const r=await fetch(`http://127.0.0.1:3200/api/audit/report-pdf?${query.replace('tier=basic','tier=advanced')}`,{signal:AbortSignal.timeout(10000)});await r.body?.cancel();assert(r.status===401,'advanced_bypass');return{http:r.status};});
