...
async function ready(port){for(let i=0;i<100;i++){try{const r=await fetch(`http://localhost:${port}/en`,{signal:AbortSignal.timeout(1500)});await r.body?.cancel();return;}catch{await sleep(300);}}throw new Error(`server_not_ready_${port}`);}
...
async function getJson(origin='http://localhost:3200',extra={}){const r=await fetch(`${origin}/api/audit/report?${query}`,{headers:extra,signal:AbortSignal.timeout(18000)});const body=await r.json();return{status:r.status,body};}
try{
 ...
 const app=launch('next-a','node',['node_modules/next/dist/bin/next','start','--hostname','localhost','--port','3100']);await ready(3100);
 await record('missing-trusted-proxy-is-503',async()=>{const {status,body}=await getJson('http://localhost:3100',{'x-forwarded-for':'203.0.113.11','x-vercel-forwarded-for':'203.0.113.11'});assert(status===503,`unexpected_unsigned_status_${status}_${body.mode??body.error}`);return{http:status,error:body.mode??body.error};});
 await record('forged-hmac-is-503',async()=>{const {status}=await getJson('http://localhost:3100',{'x-velmere-proxy-address':'203.0.113.11','x-velmere-proxy-time':String(Date.now()),'x-velmere-proxy-signature':'0'.repeat(64)});assert(status===503,`unexpected_forged_assertion_status_${status}`);return{http:status};});
 proxy=http.createServer((request,response)=>{
  ...
  // NextURL normalizes loopback IPs to localhost. Use that exact canonical host
  // at the listeners, proxy and allowlist; do not weaken the application host checks.
  headers.host=`localhost:${upstreamPort}`;
  const next=http.request({hostname:'localhost',port:upstreamPort,path:request.url,method:request.method,headers},res=>{response.writeHead(res.statusCode??502,res.headers);res.pipe(response);});
 ...
 });await new Promise(r=>proxy.listen(3200,'localhost',r));
 ...
 await record('production-anonymous-advanced-pdf-still-denied',async()=>{const r=await fetch(`http://localhost:3200/api/audit/report-pdf?${query.replace('tier=basic','tier=advanced')}`,{signal:AbortSignal.timeout(10000)});await r.body?.cancel();assert(r.status===401,`unexpected_advanced_status_${r.status}`);return{http:r.status};});
 await record('production-post-json-shares-runtime',async()=>{const r=await fetch('http://localhost:3200/api/audit/report',{method:'POST',headers:{'content-type':'application/json',origin:'http://localhost:3200'},body:JSON.stringify({address:target,chainId:'1',analysisMode:'runtime',tier:'basic'}),signal:AbortSignal.timeout(18000)});const body=await r.json();assert(r.status===200&&body.report?.runtimeAnalysis?.status==='STATIC_ANALYSIS_COMPLETED',`post_http_${r.status}_${body.mode??body.error}`);return{http:r.status};});
 const appB=launch('next-b','node',['node_modules/next/dist/bin/next','start','--hostname','localhost','--port','3101']);await ready(3101);upstreamPort=3101;
 ...
 await record('pdf-cannot-reset-exhausted-shared-quota',async()=>{const r=await fetch(`http://localhost:3200/api/audit/report-pdf?${query}`,{signal:AbortSignal.timeout(10000)});await r.body?.cancel();assert(r.status===429,`format_http_${r.status}`);return{http:r.status};});
