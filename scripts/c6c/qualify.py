"""Qualification of the checked-out real application; one log per command."""
from pathlib import Path
import subprocess,os,json,datetime,hashlib,re
out=Path('/tmp/c6c-evidence');out.mkdir(exist_ok=True)
sha=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip();os.environ['C6_SOURCE_SHA']=sha
checks=[('clean-install',['npm','ci','--ignore-scripts','--no-fund'],480),('strict-typescript',['node_modules/.bin/tsc','--noEmit','--strict','--pretty','false'],420),('eslint-zero-warning',['node_modules/.bin/eslint','.','--max-warnings','0','--format','json','--output-file',str(out/'eslint.json')],420),('dependency-audit',['npm','audit','--json'],180),('dependency-tree',['npm','ls','--all','--json'],180),('actual-route-and-regression-tests',['node_modules/.bin/tsx','--test','scripts/c6/route-boundaries.test.ts','scripts/c6/provider-engine-boundaries.test.ts','scripts/c6c/actual-app-regressions.test.ts'],240),('production-build',['npm','run','build'],720),('engine-observations',['node_modules/.bin/tsx','scripts/c6/engine-observations.ts',str(out/'engine')],240)]
...
for name,args,limit in checks:
 ...
 scope='OBSERVATION_COLLECTION_NOT_BENCHMARK_PASS' if name=='engine-observations' else 'ACTUAL_SOURCE_CHECK'
