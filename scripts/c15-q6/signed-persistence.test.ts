...
import { resetSupabaseRpcGovernorForTests } from '../../lib/db/supabase-rpc-runtime-governor';
...
// Only transports are controlled; Q7 replaces the ordering adapter with actual SQL. No real payment, GoTrue login,
// PostgREST service or HTTP socket is claimed. Grants below are fixture setup.
// Fault-injection scenarios are independent. Do not carry a tripped circuit
// from one scenario into another; runtime breaker settings stay unchanged.
beforeEach(()=>resetSupabaseRpcGovernorForTests());
...
before(async()=>{
 ...
 for(const [ack,file] of [['entitlement','../c15-q4/entitlement-store.sql'],['effect','../c15-q5/effect-store.sql'],['event','./event-store.sql'],['watermark','../c15-q7/watermark-store.sql']]){
...
async function deliver(subject:string,eventId:string,options:{type?:'charge.refunded'|'charge.dispute.created';tamper?:boolean;live?:boolean;created?:number}={}){
 ...
 const response=await handleStripeWebhookRequest(request,{...stripeWebhookIngressDependencies,webhookSecret:()=>secret,getStripe:()=>stripe,
  ...
  // Q7 uses the real ordering client and SQL, without an accept-all adapter.
  // These refund/dispute fixtures do not exercise commerce holds or actual checkout.
 });
...
// Q7 scenarios: fresh event IDs, so only ordering SQL can identify stale delivery.
test('Q7 fresh older refund after chargeback is ignored by persisted ordering',async()=>{
 await grant('q7_stale');await deliver('q7_stale','evt_q7_cb',{type:'charge.dispute.created',created:1700000002});
 const mark=calls.length;const r=await deliver('q7_stale','evt_q7_stale',{created:1700000000});assert.equal(r.status,200);assert.equal(r.body.staleIgnored,true);
 assert.equal(r.body.orderingReason,'terminal_state_dominates');assert.equal(await status('q7_stale'),'revoked');assert.equal(await count('q7_stale'),1);
 assert.equal(calls.slice(mark).includes('velmere_apply_vlm_paid_entitlement_lifecycle_event'),false);
});
test('Q7 failed ordering transport leaves retryable inbox and no effect; retry succeeds',async()=>{
 await grant('q7_order_retry');fault='velmere_apply_payment_event_watermark';const id='evt_q7_order_retry';
 assert.equal((await deliver('q7_order_retry',id)).status,503);assert.equal(await count('q7_order_retry'),0);
 assert.equal((await deliver('q7_order_retry',id)).status,200);assert.equal(await count('q7_order_retry'),1);
});
test('Q7 retry with an existing watermark completes the still-unapplied refund',async()=>{
 await grant('q7_after_watermark');fault='velmere_apply_vlm_paid_entitlement_lifecycle_event';const id='evt_q7_after_watermark';
 assert.equal((await deliver('q7_after_watermark',id)).status,500);
 const rows=(await db.query('SELECT * FROM public.velmere_payment_event_watermarks WHERE event_id=$1',[id])).rows;assert.equal(rows.length,1);
 assert.equal(await status('q7_after_watermark'),'active');const retry=await deliver('q7_after_watermark',id);assert.equal(retry.status,200,JSON.stringify({retry,inbox:(await db.query('SELECT status,attempt_count,last_error_code FROM public.velmere_stripe_webhook_events WHERE id=$1',[id])).rows}));
 assert.equal(await status('q7_after_watermark'),'refunded');assert.equal(await count('q7_after_watermark'),1);
});
