"""Qualify the frozen local Q6 scope without claiming a remote commit or release GO."""
...
def main() -> int:
    ...
    files = json.loads((root / 'scripts/c15-q6/TEST_FILES.json').read_text())
    if not isinstance(files, list) or len(files) != len(set(files)) or len(files) != 51:
        raise ValueError('Expected the frozen 51-file test manifest without duplicates.')
