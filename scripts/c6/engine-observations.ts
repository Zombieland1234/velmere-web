...
for(const sample of samples){
 ...
 const errors=(compiled.errors??[]).filter((e:any)=>e.severity==='error');if(errors.length)throw new Error(JSON.stringify(errors));
 ...
 const normalized=(x:any)=>x.findings.map((f:any)=>({id:f.findingId,severity:f.severity,confidence:f.confidence,exploitability:f.exploitability,offset:f.bytecodeOffset}));
