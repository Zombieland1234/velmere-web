...
type SyntheticPolicy={
 ...
 status:string;
 evidence:{reference:string;reviewStatus:string;verifiedAt:string;reviewBy:string;effectiveFrom:string;expiresAt:string};
 grants:Array<{datasetId:string;productId:string;environment:'internal_test';operations:string[];maxCacheTtlSeconds:number}>;
};
function fixture(change:(p:SyntheticPolicy)=>void=()=>{}) {
 const policy:SyntheticPolicy={schemaVersion:'velmere.provider-use-policy.v1',providerId:'synthetic',policyVersion:'test-1',status:'approved',evidence:{reference:'SYNTHETIC_NO_LEGAL_GRANT',reviewStatus:'verified',verifiedAt:'2026-09-16T00:00:00.000Z',reviewBy:'2026-09-18T00:00:00.000Z',effectiveFrom:'2026-09-16T00:00:00.000Z',expiresAt:'2026-09-18T00:00:00.000Z'},grants:[{datasetId:'synthetic-data',productId:'synthetic-product',environment:'internal_test',operations:['display_derived','export_derived_pdf','cache'],maxCacheTtlSeconds:60}]};change(policy);
